...
def get_gitlab_client():
    ...
    if (merge_requests.count == 0): exit()
...
def get_merge_request_dict(gitlab_client, project_id, merge_request_iid) -> dict[str, Any]:
    project = gitlab_client.projects.get(project_id)
    merge_request = project.mergerequests.get(merge_request_iid)
    merge_request_dict = merge_request.attributes
    diffs_list = get_merge_request_diffs_list(gitlab_client, project_id, merge_request_iid)
    return {
        "title": merge_request_dict["title"],
        "description": merge_request_dict["description"],
        "author": {
            "name": merge_request_dict["author"]["name"]
        },
        "diffs": diffs_list
    }
...
def get_merge_request_diffs_list(gitlab_client, project_id, merge_request_iid) -> list[dict[str, Any]]:
    project = gitlab_client.projects.get(project_id)
    merge_request = project.mergerequests.get(merge_request_iid)
    ...
    for diff in diffs:
        ...
        for diff_file_dict in diff_files_dict:
            ...
            diff_file = io.StringIO(f"""--- a/{diff_file_dict["old_path"]}\n+++ b/{diff_file_dict["new_path"]}\n{diff_file_dict["diff"]}""")
            # Analysis diff file
...
if __name__ == "__main__":
    ...
    merge_request_dict = get_merge_request_dict(gitlab_client, project_id, merge_request_iid)
    print(json.dumps(merge_request_dict, indent=2))
    print()

    print('\033[32m' + '# get_merge_request_diffs_list' + '\033[0m')
    merge_request_diffs = get_merge_request_diffs_list(gitlab_client, project_id, merge_request_iid)
